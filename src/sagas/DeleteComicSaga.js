...
function* DeleteComic(action) {
   try {
      var response = yield(deleteComicFunction(action.data.username, action.data.comic));
      if(response.message !== null && response.message !== "Deleted successfully!"){
        ...
        yield put({type: "DELETE_COMIC_SUCCESS", data: {'message': response.message, "id": response.id}});                      
      }
   } catch (e) {
     console.log(e);
...
function deleteComicFunction(username, comic){
    var myHeaders = new Headers();
    myHeaders.append("Accept", "application/json");
    myHeaders.append("Content-Type", "application/json");
    var myInit = { method: 'POST', headers: myHeaders, cache: 'default'};
    var myRequest = new Request('http://198.199.102.156:5000/DeleteComicUnread/' + username + '/' + comic.id, myInit);
