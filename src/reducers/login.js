...
export default (state = initialState, action) => {
  switch (action.type) {
    case 'LOGIN_SUCCESSFUL':
      localStorage.setItem("user", JSON.stringify({"username": action.data.username, "password": state.password, "unread": action.data.unread, "read": action.data.read}));    
    ...
    case 'SET_LOGIN':
      return state.merge({"username": action.data.username.username, "logged_in": true, "unread": action.data.username.unread, "read": action.data.username.read})
    case 'LOGOUT': 
